...
# =====================================================================
# NOTIFICHE DESKTOP (toast di Windows)
# =====================================================================
_toast_notifier = None
_toast_import_warned = False
...
def notify_desktop(title, message):
    """Mostra una notifica toast di Windows, se abilitata in config e se il
    pacchetto 'win10toast' è disponibile. Su piattaforme diverse da Windows,
    o se disabilitata, non fa nulla (nessun errore)."""
    global _toast_import_warned
    if not NOTIFICATIONS_CONFIG.get("enabled", False):
        return
    if not sys.platform.startswith("win"):
        return
    if _toast_notifier is None:
        if not _toast_import_warned:
            logger.warning(
                "[NOTIFY] Pacchetto 'win10toast' non installato: notifiche desktop disattivate. "
                "Esegui: pip install win10toast"
            )
            _toast_import_warned = True
        return
    try:
