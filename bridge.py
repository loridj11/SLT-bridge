...
# =====================================================================
# LOGGING SU FILE (con rotazione) + CONSOLE (colorata, verbosità separata)
# =====================================================================

def _enable_windows_ansi_support():
    """Su Windows abilita l'elaborazione delle sequenze ANSI nel terminale
    (necessaria sui vecchi cmd.exe; PowerShell moderno e Windows Terminal la
    supportano già di default, quindi qui non cambia nulla). Nessuna dipendenza
    esterna: usa direttamente le API Win32 tramite ctypes. Best-effort: se
    fallisce per qualunque motivo, i log restano semplicemente senza colore
    invece di generare un errore.

    Abilita l'elaborazione ANSI su ENTRAMBI gli handle di stdout e stderr:
    sono handle di console separati su Windows, e va abilitata sullo stesso
    handle su cui poi si scrive davvero — altrimenti il colore resta 'acceso'
    sull'handle sbagliato e i log continuano ad apparire senza colore."""
...
class _ColorConsoleFormatter(logging.Formatter):
    """Formatter con colori ANSI per la console: rende i log più leggibili a
    colpo d'occhio (grigio per DEBUG, bianco per INFO, giallo per WARNING,
    rosso per ERROR/CRITICAL). Il file di log resta sempre testo semplice
    (senza codici colore), tramite un formatter separato — i colori servono
    solo per la leggibilità a schermo, non ha senso salvarli su disco."""
    COLORS = {
        logging.DEBUG: "\033[90m",       # grigio
        logging.INFO: "\033[34m",        # blu
        logging.WARNING: "\033[33m",     # giallo
        logging.ERROR: "\033[31m",       # rosso
        logging.CRITICAL: "\033[1;31m",  # rosso in grassetto
    }
    RESET = "\033[0m"
...
def setup_logging(cfg):
    ...
    file_level = getattr(logging, str(log_cfg.get("level", "INFO")).upper(), logging.INFO)

    # Se console_verbose=False, la CONSOLE mostra solo WARNING/ERROR/CRITICAL,
    # ma il FILE di log continua comunque a registrare tutto a 'level' — utile
    # per avere una console pulita durante l'uso normale, senza perdere il
    # dettaglio su disco per un'eventuale analisi successiva di un problema.
    console_verbose = log_cfg.get("console_verbose", True)
    console_level = file_level if console_verbose else logging.WARNING

    lg = logging.getLogger("bridge")
    # Il logger deve lasciar passare il livello PIÙ permissivo tra i due handler:
    # sono poi i singoli handler (file/console) a filtrare ulteriormente.
    lg.setLevel(min(file_level, console_level))
    lg.propagate = False
    lg.handlers.clear()  # evita handler duplicati se il modulo viene ricaricato

    plain_fmt = logging.Formatter("%(asctime)s [%(levelname)s] %(message)s", "%Y-%m-%d %H:%M:%S")
    ...
    file_handler.setFormatter(plain_fmt)  # niente codici colore nel file su disco
    lg.addHandler(file_handler)

    # 'color_console' attiva/disattiva il TENTATIVO di colorare la console;
    # normalmente si attiva SOLO se sys.stdout.isatty() rileva un terminale
    # reale (per non sporcare l'output quando è rediretto su file/pipe).
    # 'force_color_console' bypassa del tutto il rilevamento automatico e
    # forza sempre i colori: utile se il proprio terminale (es. capita con
    # alcune combinazioni PowerShell + Windows Terminal) non viene rilevato
    # correttamente come TTY da Python pur essendolo a tutti gli effetti.
    isatty_detected = sys.stdout.isatty()
    force_color = log_cfg.get("force_color_console", False)
    use_color = log_cfg.get("color_console", True) and (isatty_detected or force_color)
    if use_color:
        _enable_windows_ansi_support()
    color_fmt = _ColorConsoleFormatter(
        "%(asctime)s [%(levelname)s] %(message)s", "%Y-%m-%d %H:%M:%S", use_color=use_color
    )
    ...
    # Diagnostica per capire subito, dai log, perché i colori sono attivi o meno
    # (utile via file di log anche quando console_verbose=False nasconde l'INFO).
    lg.info(
        f"[LOG DIAGNOSTICA] color_console={log_cfg.get('color_console', True)} | "
        f"force_color_console={force_color} | sys.stdout.isatty()={isatty_detected} | "
        f"colori attivi={use_color} | piattaforma={sys.platform}"
    )
...
if __name__ == '__main__':
    logger.info("==================================================")
    logger.info(f" Bridge Spotify Lyrics -> LM Studio (Parallel 4) - v{__version__}")
    logger.info(f" In ascolto su: http://{SERVER_CONFIG.get('host', '127.0.0.1')}:{SERVER_CONFIG.get('port', 5000)}")
    logger.info("==================================================")
